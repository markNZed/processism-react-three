...
const ParticlesInstance = React.forwardRef(({ id, node, config }, ref) => {
    ...
    useEffect(() => {
        ...
        renderBlobRef.current.circleMaterial = createCircleShaderMaterial(particleRadius);
        renderBlobRef.current.circleGeometry = createCircleShaderGeometry(particleRadius, particles.length, maxParticleCount);        
    }, []);
